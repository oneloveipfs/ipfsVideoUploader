...
let configInterpreter = fs.readFileSync(process.cwd()+'/src/config.js',{encoding: 'utf8'}).toString()
...
if (process.env.REMOTE_APP == '1') {
    ...
    configInterpreter = configInterpreter.replace("const REMOTE_APP = 0","const REMOTE_APP = 1")
} else {
    ...
    configInterpreter = configInterpreter.replace("const REMOTE_APP = 1","const REMOTE_APP = 0")
}

fs.writeFileSync(process.cwd()+'/src/electronApp.js',electronJs,'utf8')
fs.writeFileSync(process.cwd()+'/src/config.js',configInterpreter,'utf8')