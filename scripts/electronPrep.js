...
if (process.env.REMOTE_APP == '1') {
    ...
    electronJs = electronJs.replace("const config = require('./config')","const config = require('../remoteAppConfig.json')")
    ...
    electronJs = electronJs.replace("const config = require('../remoteAppConfig.json')","const config = require('./config')")
